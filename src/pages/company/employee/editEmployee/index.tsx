...
export default function EmployeeEdit() {
  ...
  try {
    // Usando query param para buscar nome ou ID
...
const handleUpdate = async (e: FormEvent) => {
  e.preventDefault();
  if (!foundEmployee?.id) {
    alert("Selecione um funcionário válido antes de atualizar");
    return;
  }
  ...
  try {
    const body = {
      ...
      employee_roles: foundEmployee.employee_roles, // envia conforme a API espera
    };

    const response = await api.put(`/employee/${foundEmployee.id}`, body);
