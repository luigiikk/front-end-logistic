...
type DeletePurchaseItemModalProps = {
  name: string;
  onCancel: () => void;
  onConfirm: () => void;
};

export function DeletePurchaseItemModal({
  name,
  onCancel,
  onConfirm,
}: DeletePurchaseItemModalProps) {
  return (
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 flex flex-col items-center text-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            <span className="font-semibold text-gray-600">{name}</span> será
